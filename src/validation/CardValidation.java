 ...
 * This class includes methods to validate credit card numbers using the Luhn algorithm,
 * as well as additional validations for expiration dates and CVV codes.
 *
 * @version 1.0
 * @since 2024-05-20
 * @author Aarón Rojas
...
public class CardValidation {
    ...
    /**
     * Checks if the credit card number matches known patterns for Visa, MasterCard, or Amex.
     * 
    ...
    private void isValidExpiryDate(String date) throws CardException {
        ...
        // Additional date validation logic can be added here, e.g., checking if the date is in the future
    }
    ...
    private void isValidCVV(String cardNumber, int cvv) throws CardException {
        ...
        if ((VISA_PATTERN.matcher(cardNumber).matches() || MASTERCARD_PATTERN.matcher(cardNumber).matches()) && cvvStr.length() != 3) {
